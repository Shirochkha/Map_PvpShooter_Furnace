    ...
    /* void FixedUpdate()
     {
         ...
         if (playerCanMove)
         {
             // Рассчитываем целевую скорость движения
             Vector3 targetVelocity = new Vector3(Input.GetAxis("Horizontal"), 0, Input.GetAxis("Vertical"));
             targetVelocity = transform.TransformDirection(targetVelocity);

             // Проверяем, движется ли игрок и находится ли он на земле
             if ((targetVelocity.x != 0 || targetVelocity.z != 0) && isGrounded)
             {
                 isWalking = true;
             }
             ...
             RaycastHit hit;
             float slopeAngle = 0f;
             if (Physics.Raycast(transform.position, Vector3.down, out hit, 3f)) // Увеличиваем расстояние до 3
             {
                 slopeAngle = Vector3.Angle(hit.normal, Vector3.up);
                 Debug.DrawRay(hit.point, hit.normal, Color.green); // Визуализация нормали
             }

             float maxSlopeAngle = 5f; // Устанавливаем максимальный угол наклона

             //Debug.Log(slopeAngle);

             // Проверяем, если угол наклона превышает допустимый
             if (slopeAngle > maxSlopeAngle)
             {
                 // Ограничиваем вертикальную скорость, чтобы игрок не поднимался
                 rb.velocity = new Vector3(rb.velocity.x, Mathf.Min(rb.velocity.y, 0), rb.velocity.z);
             }
             else
             {
                 // Все расчеты движения при активном беге
                 if (enableSprint && Input.GetKey(sprintKey) && sprintRemaining > 0f && !isSprintCooldown)
                 {
                 ...
                 // Все расчеты движения при ходьбе
                 else
                 {
                     isSprinting = false;
